...
import { AddSabotierInState } from "../../redux/slice/globalSlice";
import { CREATE_SABOTIER } from "../../graphql/queries";
...
export default function PopUpAddPointer({ openModal, setModalON }) {
  const [createPointer, { data, error, loading, reset }] =
    useMutation(CREATE_SABOTIER);
  const dispatch = useAppDispatch();
  const handleClose = () => setModalON(false);

  useEffect(() => {
    if (data) {
      const { createSabotier } = data;
      if (createSabotier) {
        dispatch(AddSabotierInState(createSabotier));
      }
    }
