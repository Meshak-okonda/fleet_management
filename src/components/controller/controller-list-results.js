...
import { DELETE_CONTROLLER, RECOVERY_CONTROLLER } from "../../graphql/queries";
import {
...
export const SabotierListResults = () => {
  ...
  const [driverView, setDriverView] = useState(controllers);
  ...
  const handleChangeView = (event, newValueView) => {
    if (newValueView === "1") {
      setPage(0);
      setDriverView(controllers.filter((poi) => poi.delete === false));
      setRowCount(controllers.filter((poi) => poi.delete === false).length);
      ...
      setPage(0);
      setDriverView(controllers);
      setRowCount(controllers.length);
    }
  ...
  useEffect(() => {
    if (data) {
      if (data.reverseDeleteController) {
        dispatch(
          updateControllerInState({
            ...data.reverseDeleteController,
            delete: false,
          })
        );
        setValueView("1");
      }
      if (data.deleteController) {
        dispatch(
          updateControllerInState({ ...data.deleteController, delete: true })
        );
        setValueView("1");
      }
    }
  ...
  const handlePageChange = (event, newPage) => {
    ...
    if (valueView === "1") {
      ...
      setPage(0);
      setDriverView(controllers.slice(leftLimit, leftLimit + limit));
    }
    setPage(newPage);
  };
  ...
  const confirmMutation = async (mutation) => {
    try {
      await mutation({
        variables: {
          deleteControllerId: idSelect,
        },
      });
      deleteControllerId(false);
    } catch (error) {
  ...
  const confirmMutationRecovery = async (mutation) => {
    try {
      await mutation({
        variables: {
          reverseDeleteControllerId: idSelect,
        },
      });
  ...
  const handleSuperAdmSearch = (value) => {
    ...
    setDriverView(driverFilter.length < 1 ? controllers : driverFilter);
  };
  ...
  const handleSearch = (value) => {
    ...
    setDriverView(
      driverFilter.length < 1
        ? controllers.filter((poi) => poi.delete === false)
        : driverFilter
    );
  };
      ...
      <Card>
        ...
        {modalUpdateOn && (
          <PopUpModifyPointer
            ...
            updateDriver={controllers.find((poi) => poi.id === idSelect)}
          />
        )}
      </Card>
    </>
