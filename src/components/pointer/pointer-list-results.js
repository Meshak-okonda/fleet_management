...
import { DELETE_POINTER, RECOVERY_POINTER } from "../../graphql/queries";
...
export const PointerListResults = () => {
  ...
  const [driverView, setDriverView] = useState(pointers);
  ...
  const handleChangeView = (event, newValueView) => {
    if (newValueView === "1") {
      setPage(0);
      setDriverView(pointers.filter((poi) => poi.delete === false));
      setRowCount(pointers.filter((poi) => poi.delete === false).length);
      ...
      setPage(0);
      setDriverView(pointers);
      setRowCount(pointers.length);
    }
  ...
  useEffect(() => {
    if (data) {
      if (data.reverseDeletePointer) {
        dispatch(
          updatePointerInState({ ...data.reverseDeletePointer, delete: false })
        );
        setValueView("1");
      }
      if (data.deletePointer) {
        dispatch(updatePointerInState({ ...data.deletePointer, delete: true }));
        setValueView("1");
      }
    }
  }, [data, dispatch]);
  ...
  const handlePageChange = (event, newPage) => {
    ...
    if (valueView === "1") {
      ...
      setPage(0);
      setDriverView(pointers.slice(leftLimit, leftLimit + limit));
    }
    setPage(newPage);
  };
  ...
  const confirmMutationRecovery = async (mutation) => {
    try {
      await mutation({
        variables: {
          reverseDeletePointerId: idSelect,
        },
      });
  ...
  const handleSuperAdmSearch = (value) => {
    ...
    setDriverView(driverFilter.length < 1 ? pointers : driverFilter);
  };
  ...
  const handleSearch = (value) => {
    ...
    setDriverView(
      driverFilter.length < 1
        ? pointers.filter((poi) => poi.delete === false)
        : driverFilter
    );
  };
      ...
      <Card>
              ...
              <TableBody>
                {driverView.slice(0, limit).map((pointer, idx) => (
                  ...
                  >
                    ...
                    <TableCell>{pointer.age}</TableCell>
        ...
        {modalUpdateOn && (
          <PopUpModifyPointer
            ...
            updateDriver={pointers.find((poi) => poi.id === idSelect)}
          />
        )}
      </Card>
    </>
