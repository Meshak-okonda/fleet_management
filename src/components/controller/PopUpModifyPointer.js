...
import { updateSabotierInState } from "../../redux/slice/globalSlice";
import { UPDATE_CONTROLLER } from "../../graphql/queries";
...
export default function PopUpModifyDriver({
  ...
  const [updatePointer, { data, error, loading, reset }] =
    useMutation(UPDATE_CONTROLLER);
  const dispatch = useAppDispatch();
  const handleClose = () => setModalON(false);

  useEffect(() => {
    if (data && data.updateSabotier) {
      dispatch(updateSabotierInState(data.updateSabotier));
      setModalON(false);
    }
  ...
  async function onSubmit(data) {
    await updatePointer({
      variables: {
        updateControllerId: updateDriver.id,
        controller: { ...data },
      },
    });
  }
