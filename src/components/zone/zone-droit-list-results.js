...
export const ZoneRight = () => {
  ...
  const [driverView, setDriverView] = useState(rightPlage);
  ...
  const handleChangeView = (event, newValueView) => {
    if (newValueView === "1") {
      setPage(0);
      setDriverView(rightPlage.filter((poi) => poi.delete === false));
      setRowCount(rightPlage.filter((poi) => poi.delete === false).length);
      ...
      setPage(0);
      setDriverView(rightPlage);
      setRowCount(rightPlage.length);
    }
    setValueView(newValueView);
  };

  useEffect(() => {
    setRightPlage(plages.filter((e) => e.zone == "DROIT"));
  }, [plages]);
  useEffect(() => {
    setDriverView(rightPlage.filter((e) => e.zone == "DROIT"));
  }, [rightPlage]);
  ...
  const handlePageChange = (event, newPage) => {
    ...
    if (valueView === "1") {
      ...
      setPage(0);
      setDriverView(rightPlage.slice(leftLimit, leftLimit + limit));
    }
    setPage(newPage);
  };
  ...
  const handleSuperAdmSearch = (value) => {
    ...
    setDriverFilter(
      rightPlage.filter((poi) => {
        if (`${poi.name.toUpperCase()}}`.indexOf(value.toUpperCase()) > -1)
          return poi;
      })
    );
    setDriverView(driverFilter.length < 1 ? rightPlage : driverFilter);
  };
  ...
  const handleSearch = (value) => {
    ...
    setDriverView(
      driverFilter.length < 1
        ? rightPlage.filter((poi) => poi.delete === false)
        : driverFilter
    );
  };
  ...
  return (
    <>
      <Box sx={{ mt: 1, mb: 1 }}>
        <Card sx={{ py: 0 }}>
          <CardHeader title="Zone Droite" sx={{ py: 1 }} />
      ...
      <Card>
        ...
        {modalUpdateOn && (
          <PopUpModify
            ...
            update={rightPlage.find((poi) => poi.id === idSelect)}
          />
        )}
      </Card>
    </>
