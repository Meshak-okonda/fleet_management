...
import { AddPointerInState } from "../../redux/slice/globalSlice";
import { CREATE_POINTER } from "../../graphql/queries";
...
export default function PopUpAddPointer({ openModal, setModalON }) {
  const [createPointer, { data, error, loading, reset }] =
    useMutation(CREATE_POINTER);
  const dispatch = useAppDispatch();
  const handleClose = () => setModalON(false);

  useEffect(() => {
    if (data) {
      const { createPointer } = data;
      if (createPointer) {
        dispatch(AddPointerInState(createPointer));
      }
    }
